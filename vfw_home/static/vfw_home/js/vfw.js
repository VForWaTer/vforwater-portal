...
function drawOnMapMenu(test) {
    ...
    drawSquare.on('drawend', function () {
        ...
        selectedFeatures.getArray().forEach(function (val) {
            vfw.var.obj.selectedIds.map.push(parseInt(val.getId().replace(append_str, '')))  // PaulsLayer1 to int(1)
        });
        if (vfw.var.obj.selectedIds.map.length > 0) {
            mapSelectFunction(vfw.var.obj.selectedIds.map);
        }*/
