...
class DataObject:
    ...
    def __set_timescale__(self):
        ...
        if timescale:
            ...
            self.__timescale_from_data__(self)
    ...
    def __get_gap_pos__(self):
        ...
        perfect_frame = pd.DataFrame(pd.date_range(start=starttime, end=endtime, freq=self.timescale), columns=['tstamp'])
    ...
    def __fill_data_gaps__(self):
        ...
        if 'avg' in self.dataframe.columns:  # if dataset with average, min, max  values
            for pos in self.__value_before_gap__[::-1]:
                ...
                self.dataframe[col][2] = self.dataframe[col][2][: pos + 1] + (bandbef, bandaft,) + self.dataframe[col][2][
                                                                                           pos + 1:]
                self.dataframe[col][3] = self.dataframe[col][3][: pos + 1] + (bandbef, bandaft,) + self.dataframe[col][3][
                                                                                           pos + 1:]
            ...
            for pos in self.__value_before_gap__[::-1]:
                ...
                empty_rows['tstamp'] = self.dataframe['tstamp'][pos] + self.timescale, self.dataframe['tstamp'][pos + 1] - self.timescale
                ...
                defect_x.extend([self.dataframe['tstamp'][pos] - self.timescale, self.dataframe['tstamp'][pos],
                                 self.dataframe['tstamp'][pos + 1], self.dataframe['tstamp'][pos + 1] + self.timescale])
                if self.data_format == '3D':
                    defect_y.extend([float('nan'), float('nan'), float('nan'), float('nan'), ])
                elif 'value' in self.dataframe.columns:
                    defect_y.extend([float('nan'), self.dataframe['value'][pos], self.dataframe['value'][pos + 1], float('nan'), ])
                elif 'data' in self.dataframe.columns:
                    defect_y.extend([float('nan'), self.dataframe['data'][pos], self.dataframe['data'][pos + 1], float('nan'), ])
