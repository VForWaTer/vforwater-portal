...
from vfwheron.models import Keywords, EntrygroupTypes, Variables, Licenses, Entries, Details, Persons, BasicFilter, \
    NmPersonsEntries, TemporalScales, Entrygroups, Datasources
...
class Table:
    ...
    def build_json_child(self):
        ...
        for item in self.child_columns:
            items = {}
            map_items = {}
            recursive = False
            if item in self.child and item is not None:
                # build different menus according to the type defined in the model:
                if item in self.filter_type:
                    switch = self.filter_type[item]

                    if switch == 'slider':
                        result = self.__build_slider_json(item)
                    elif switch == 'date':
                        result = self.__build_date_json(item)
                    # Recursive is one single table, so the build process is highly customized to that single table
                        ...
                        # can help
                        result = self.__build_recursive_json(item)
                        recursive = True
                    # elif switch == 'draw':
                    #     result = self.__build_draw_json(item)
                    elif switch == 'bool':
                        result = self.__build_bool_json(item)
                else:
                    result = self.__build_default_json(item)
                ...
                if recursive:
                    ...
                    items.update(
                            dict(name=self.table_name.db_alias_child[item], total=result['total']))
                    map_items.update({
                        'name': self.table_name.db_alias_child[item],
                        'column': item,
                        })

                    items.update(result['json'])
                    map_items.update(result['server'])

                    if result['total'] >= self.min_amount:
                        counter = counter + 1
                        json_all_childs.update({'C' + str(counter): items})
                        map_all_childs.update({'C' + str(counter): map_items})
    ...
    def __build_default_json(self, item):
        """
        ...
        map_items = {}
        counter = 0
        for values in self.child[item]:
            if values is not None:
                filtermap = {'{0}'.format(self.query_paths[item]): values}
                ...
                if total >= self.min_amount:
                    counter = counter + 1
                    all_items.update({'I' + str(counter): item_dict})
                    map_items.update({'I' + str(counter): map_item_dict})
        ...
        # print('---', '\033[0m')
        return {'json': all_items, 'total': counter, 'server': map_items}

    def __build_bool_json(self, item):
        """
        ...
        if self.child[item]:
            ...
            for i, case in enumerate(values):
                filtermap = {'{0}'.format(self.query_paths[item]): i}
                ...
                all_items.update({'type': 'bool',
                                        'I' + str(i+1): item_dict})
                map_items.update({'type': 'bool',
                                        'I' + str(i+1): map_item_dict})
            ...
            # print('---', '\033[0m')
        return {'json': all_items, 'total': total[1], 'server': map_items}

    def __build_slider_json(self, item):
        """
        ...
        """
        counter = 0
        total = len(self.child[item])
        keyword = self.query_paths[item]
        ...
        if total >= self.min_amount:  # min amount for debugging
            counter = counter + 1
        return {'json': item_dict, 'total': counter, 'server': map_item_dict}

    def __build_date_json(self, item):
        """
        ...
        counter = 0
        total = len(self.child[item])
        d_min = {'{0}'.format('min_value'): Min(self.query_paths[item])}
        d_max = {'{0}'.format('max_value'): Max(self.query_paths[item])}
    ...
    def __build_recursive_json(self, grand_child):
        """
        recursive not used in metacatalog 0.2. Maybe obsolete
        :param grand_child:
        :type grand_child:
        :return:
        :rtype:
        ...
        for project in project_values:
            ...
            for child in recursive_child:
                ...
                for grandchild in grandchild_values:
                    ...
                    # grandchild_total = Entries.objects.using('mcdev').filter(nmmetadomain__domain__project_id=project_id,
