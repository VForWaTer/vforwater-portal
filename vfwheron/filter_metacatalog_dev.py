...
from vfwheron.models_metacatalog_dev import Keywords, EntrygroupTypes, Variables, Licenses, Entries, Details, Persons, BasicFilter
...
class Menu:
    ...
    def user_query_set(self):
        ...
        if self.user == 'default':
            query_set = Entries.objects.using('mcdev').filter(embargo=False).all()
            # for i in query_set:
            # return
            # print('i: ', i)
    ...
    def get_menu(self):
        ...
        #           i.variable.keyword)
        from vfwheron.models_metacatalog_dev import BasicFilter
...
class Table:
    ...
    def get_query_set(self):
        ...
        for columns in self.child_columns:
            ...
            query_set = list(self.table_name.objects.using('mcdev').select_related().distinct().exclude(**excluder).
    ...
    def build_default_json(self, grand_child):
        ...
        for values in self.child[grand_child]:
            if values is not None:
                ...
                total = Entries.objects.using('mcdev').filter(**filtermap).count()
                grandchild_dict = {
                    'name': values,
                    'total': total,
                    # 'chosen': False,
    ...
    def build_slider_json(self, grand_child):
        ...
        try:
            ...
            min_max = Entries.objects.using('mcdev').exclude(**excl_nan).exclude(**excl_none).aggregate(**c_min, **c_max)
            ...
            min_max = Entries.objects.using('mcdev').aggregate(**c_min, **c_max)
    ...
    def build_date_json(self, grand_child):
        ...
        min_max = Entries.objects.using('mcdev').aggregate(**d_min, **d_max)
        # min_max = eval("TblMeta.objects.aggregate(min_value=Min('" + self.query_paths[grand_child] + "'),
        #   max_value=Max('" + self.query_paths[grand_child] + "'))")
        grandchild_dict = {
            'type': 'date',
    ...
    def build_recursive_json(self, grand_child):
        ...
        """
        # Recursive is one single table, so the build process is highly customized to that single table
        parent_queryset = eval(self.table_name.mother + '.objects..using("mcdev")all()')
